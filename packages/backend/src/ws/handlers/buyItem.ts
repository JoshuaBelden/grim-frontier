...
export async function handleBuyItem(context: HandlerContext, payload: unknown): Promise<void> {
  ...
  const updatedInventory = addToInventory(npc.inventory ?? [], storeItem.name, storeItem.weight, storeItem.traits, storeItem.category)
...
function addToInventory(inventory: InventoryItem[], name: string, weight?: number, traits?: string[], category?: string): InventoryItem[] {
